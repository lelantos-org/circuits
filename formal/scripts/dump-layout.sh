...
#
# `formal/layout-2x2.txt` is consumed by `src/test/formal/layout_parity.test.ts`, which
# checks it against the SDK. This script checks it against Lean.
#
# Regenerate after an intentional layout change:  formal/scripts/dump-layout.sh --update
set -euo pipefail

cd "$(dirname "$0")/.."
EXPECTED=layout-2x2.txt
ACTUAL=$(mktemp)
