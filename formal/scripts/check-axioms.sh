...
#
# Regenerate after an intentional change:  formal/scripts/check-axioms.sh --update
set -euo pipefail
...
lake env lean Lelantos/Assumptions.lean 2>/dev/null \
  | tr -d '\n' \
  | sed 's/'"'"'Lelantos/\n'"'"'Lelantos/g' \
  | sed 's/  */ /g' \
  | sed '/^$/d' \
...
if [ ! -s "$ACTUAL" ]; then
  echo "FAIL: no axiom output -- 'lake env lean Lelantos/Assumptions.lean' produced nothing."
...
if ! diff -u "$EXPECTED" "$ACTUAL"; then
  ...
  echo "  formal/scripts/check-axioms.sh --update"
  exit 1
fi
