...
over these signals is in `Lelantos.Transact`.

Keeping the layout in its own module has one purpose: it is the highest-risk piece of hand
transcription in the development, it is dumped and diffed by `formal/scripts/dump-layout.sh`,
...
use come from the same definition, `formal/scripts/dump-layout.sh` is a cross-check against
...
/-- The layout as a list of slot names, for `formal/scripts/dump-layout.sh`. -/
