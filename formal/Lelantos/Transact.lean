...
from reaching conservation and every other consequence. See `Lelantos.Poseidon`.
...
* Everything is modulo the axioms in `Lelantos.Assumptions`.
-/
...
into `transact_sound` or anything else; `Lelantos.Poseidon`'s module note records why the
