...
below and `formal/scripts/check-prime.py` for the external check. Everything else about
...
Recorded as an axiom instead. It is listed in `formal/expected-axioms.txt` and surfaced by
`#print axioms`, so it can never be forgotten.

**Discharge:** `python3 formal/scripts/check-prime.py`, or `openssl prime <p>`. `p` is the
