...
// Baby-Jubjub, since a = 168700 is a quadratic residue and d = 168696 is not,
// so 1 ± d·τ never vanishes for on-curve operands.
//
...
//
// A literal rather than a function of N_BITS, since circom requires array
// lengths inside a `function` to be constant and a function's own parameters do
// not qualify.
...
// reads a prefix of the same table.
//
...
template FixedBaseMulBits(N_BITS, COEFS) {
    ...
    // Reads a prefix of the FIXED_BASE_WINDOWS() windows COEFS carries. N_BITS
    // is a template parameter, so this folds away at compile time.
    ...
    for (var i = 0; i < nWindows; i++) {
        ...
        // coordinate, sharing the four products above.
        var accLoX = 0;
        var accHiX = 0;
        var accLoY = 0;
        var accHiY = 0;
...
//
// Owns its Num2Bits, so a caller cannot drop the selector booleanity the table
// lookup depends on.
//
// N_BITS must satisfy 2^N_BITS < p for the decomposition to be alias-free, which
// holds at the 252 bits MulH uses.
