...
// Not the deployed shape (see 3x3.circom): a third instantiation of Transact,
// for consumers that need four shielded slots per side.
//
...
// overload for this shape — no such overload exists yet, so this shape is not
// verifiable on-chain until one is added:
//     [ 0]      merkle_root
//     [ 1.. 4]  nullifier[0..3]
//     [ 5.. 8]  out_cm[0..3]
//     [ 9]      public_asset_id
...
//
// This shape does NOT fit the 2^16 FFT domain the other three share: its count
// is in budget.json against a 2^17 domain, so `setup-4x4` fetches its own ptau
// and proving costs roughly twice a 3x3 proof.
//
