...
- [`TreeUpdateBatch(DEPTH, MAX_N)`](tree_update_batch.circom),
  ...
  inserting up to `MAX_N=8` pairs (16 leaves) over a relayer-supplied
  frontier. See §16.
...
`cv = value · V^t + rcv · H`; per-asset value conservation is
enforced as Edwards-point equality across the spent and output
bundles.

Audit checklist + circomspect baseline:
...
`public_asset_id` via `HashToAssetGen`.
...
[`2x2.circom`](2x2.circom) is a wiring layer: it instantiates
`SpentNote` per input, `OutputNote` per output, feeds the exposed `rH`
components plus the public `cv`s into `PerAssetPointBalance`, and
binds `out_cv_dep[j]` to `OutputNote.cv_dep`.
...
asset_ids too. `TAG_ASSET` (one byte) gives the hash a domain string
distinct from any other Pedersen call sharing Baby-Jubjub. 72 bits →
1 Pedersen segment (`BASE[0]`); the `H` base used by `ValueCommit`
is `BASE[2]`, outside the image of the 72-bit `HashToAssetGen`.
...
```mermaid
flowchart BT
    L0["leaf = Poseidon(TAG_LEAF, cm, cv_dep_x, cv_dep_y)"] --> N1["Poseidon(TAG_MERKLE, c0..c3)"]
    S0["sibling 0"] --> N1
    S1["sibling 1"] --> N1
    S2["sibling 2"] --> N1
    N1 --> N2["× DEPTH = 10"]
...
At depth 10 the tree holds `4^10 = 1,048,576` leaves.
...
- The transparent bucket is single-asset per tx.
- At `N_IN = N_OUT = 2`, at most two asset ids per side. Larger mixes
  require recompiling with larger `N_IN` / `N_OUT`.
...
0. **Fiat-Shamir.** Flatten the logical PIs in the canonical slot
   order (§2a) into the 30-entry `uint256` array, derive
...
See [../SECURITY.md](../SECURITY.md) for the full pre-merge checklist
including the `tree_update_batch` contract obligations.
...
sequenceDiagram
    ...
    V-->>C: ok
    C->>C: spent[nf] = true, insert out_cm[], forward cv_dep, pay public_out
    C-->>R: event
```
...
of truth across the in-circuit hash sites and `test/helpers.ts`. Tag
...
| [`tree_update_batch.circom`](tree_update_batch.circom) | Relayer batch tree-advance circuit (frontier-bound). See §16. |
...
## 16. `TreeUpdateBatch(DEPTH, MAX_N)` — relayer batch tree-advance proof
