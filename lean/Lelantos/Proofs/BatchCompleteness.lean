...
theorem act_sat (k : ℕ) (hk : k < 4) :
    LessThanSat (countBits + 1) ((k : ℕ) : F) ((filled : ℕ) : F) (actBits k) (act k) :=
  lessThan_witness (show k < 2 ^ (countBits + 1) by norm_num; omega) (by norm_num)

/-- The activity vector, evaluated: the first `filled` slots are active, the rest padding. -/
theorem act_eq (k : ℕ) (hk : k < 4) : act k = if k < filled then 1 else 0 := by
  interval_cases k <;> norm_num [act, actBits, natBits]

/-- The padding constraints. An active slot zeroes the `1 - active` factor; the one padding
slot carries zero in every per-leaf field, which is the `hx` hypothesis. -/
theorem pad_mul {x : ℕ → F} (hx : x 3 = 0) (k : ℕ) (hk : k < 4) :
    (1 - act k) * x k = 0 := by
  rw [act_eq k hk]
  by_cases h : k < filled
  · simp [h]
  · have hk3 : k = 3 := by simp only [filled] at h; omega
    subst hk3
    simp [hx]

/-! ## Insertion indices

`start_index = 0`, so slot `k` inserts at index `k`. With `k < 4` that is a single non-zero
quaternary digit, at the lowest level only.
-/

/-- The quaternary digit of index `k` at level `d`. -/
def dig (k d : ℕ) : ℕ := if d = 0 then k else 0

theorem dig_lt {k : ℕ} (hk : k < 4) (d : ℕ) : dig k d < 4 := by
  unfold dig; split
  · exact hk
  · norm_num

/-- Above the lowest level the index has run out of bits. -/
theorem index_bit_high {k : ℕ} (hk : k < 4) {i : ℕ} (hi : 2 ≤ i) : natBits k i = 0 := by
  have h4 : (4 : ℕ) ≤ 2 ^ i := by
...
theorem dig_eq {k : ℕ} (hk : k < 4) (d : ℕ) :
    natBits k (2 * d) + 2 * natBits k (2 * d + 1) = ((dig k d : ℕ) : F) := by
  rcases Nat.eq_zero_or_pos d with hd | hd
  · subst hd
    simp only [dig, Nat.mul_zero, Nat.zero_add]
    interval_cases k <;> norm_num [natBits]
  · rw [index_bit_high hk (by omega), index_bit_high hk (by omega)]
    simp [dig, Nat.ne_of_gt hd]

/-! ## Leaves

Three distinct commitments and a zeroed padding slot. `cv_dep` is the identity's coordinates
throughout, which keeps the padding constraints on the fourth slot satisfiable without
making the first three degenerate.
-/

/-- Slot `k`'s note commitment. Distinct across the filled slots; zero on the padding one,
which is what `pad_cm` requires. -/
def cm (k : ℕ) : F := if k < filled then ((k : ℕ) : F) + 7 else 0

theorem cm_pad : cm 3 = 0 := by norm_num [cm]
...
theorem batch_chain_sat : BatchChainSat countBits batch where
  ...
  leaf_def k _ := rfl
  idx_bits k hk := by
    -- `start_index = 0`, so the value decomposed is just `k`, and `k < 4 ≤ 2 ^ 20`.
    have hk4 : k < 4 := hk
    have hlt : k < 2 ^ (2 * depth) := by
      have h : (4 : ℕ) ≤ 2 ^ (2 * depth) := by norm_num
      omega
...
constraint system of `TreeUpdateBatch(10, 4)`, so every `BatchChainSat … → P` has a
non-empty domain. -/
theorem batchSat_satisfiable : ∃ w : BatchSignals 10 4, BatchSat 2 w :=
  ⟨BatchWitness.batch, BatchWitness.batch_sat⟩

/-- …and it is not the degenerate full batch: three leaves in four slots, so the padding
...
theorem batch_advances_witness :
    (∀ k, k < BatchWitness.batch.actualCount.val →
      InsertsTo 10 (BatchWitness.batch.leaves k) (BatchWitness.batch.idxDig k)
...
theorem batch_count_range_witness :
    1 ≤ BatchWitness.batch.actualCount.val ∧ BatchWitness.batch.actualCount.val ≤ 4 :=
